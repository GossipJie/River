        ...
        <div class="pape_title">
              <span><?php echo ($name); ?></span>
                       <ol class="breadcrumb" >
                        	...
                        	<li><a href="<?php echo leuu('portal/list/index',array('id'=>$term_id,'pid'=>$pid,'oid'=>$oid));?>"><?php echo ($name); ?></a></li>
                      </ol>
               </div>
               
               	<?php $lists = sp_sql_posts_paged("cid:$cat_id;order:post_date DESC;",10); ?>
                <ul class="item1_news_list">
                	<?php if(is_array($lists['posts'])): $i = 0; $__LIST__ = $lists['posts'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li><i class="fa fa-sign-out"></i><a href="<?php echo leuu('article/index',array('id'=>$vo['tid'],'pid'=>$pid,'oid'=>$oid));?>"><?php echo ($vo["post_title"]); ?></a><span><?php echo ($vo["post_date"]); ?></span></li><?php endforeach; endif; else: echo "" ;endif; ?>
            		<div class="clear"></div>
                </ul>
